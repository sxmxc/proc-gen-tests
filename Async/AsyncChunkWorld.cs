...
public partial class AsyncChunkWorld : Node2D
{
	[Signal]
	...
	{
		var playerChunk =  (Vector2i)_player.Position / (ChunkSize * WorldTileset.TileSize);
		var chunksToLoad = new List<Vector2i>();
		chunksToLoad.Add(playerChunk);
		chunksToLoad.Add(playerChunk + Vector2i.Right);
        chunksToLoad.Add(playerChunk + Vector2i.Left);
        chunksToLoad.Add(playerChunk + Vector2i.Up);
        chunksToLoad.Add(playerChunk + Vector2i.Down);
        chunksToLoad.Add(playerChunk + new Vector2i(1, 1));
        chunksToLoad.Add(playerChunk + new Vector2i(-1, 1));
        chunksToLoad.Add(playerChunk + new Vector2i(1, -1));
        chunksToLoad.Add(playerChunk + new Vector2i(-1, -1));

		foreach (var key in chunksToLoad)
		{
			if (_chunks.ContainsKey(key))
			{
				if (!_chunks[key].IsInsideTree())
				{
                    _mapChunkContainer.AddChild(_chunks[key]);
				}
		...
		foreach (var x in Range(NumberChunksX))
		{
			foreach (var y in Range(NumberChunksY))
			{
				...
				if (!chunksToLoad.Contains(index))
				{
					_chunks[index].Visible = false;
				}
			}
		}
